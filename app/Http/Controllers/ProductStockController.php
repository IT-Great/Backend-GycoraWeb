...
class ProductStockController extends Controller
{
     ...
     * Mengambil semua produk beserta detail batch stoknya.
     */
    public function index(Request $request) // 🌟 [PERBAIKAN 1]: Tangkap Request dari URL
    {
        // 🌟 [PERBAIKAN 2]: Baca parameter 'status' dari frontend, default ke 'active'
        $status = $request->query('status', 'active');

        // Buat Query Builder dengan Filter Status
        $products = Product::with(['category', 'stocks' => function($q) {
            $q->where('quantity', '>', 0);
        }])
        ->where('status', $status) // 🌟 [PERBAIKAN 3]: Filter berdasarkan status
        ->latest()
        ->get();
    ...
    public function store(Request $request, $productId)
    {
        ...
        try {
            // Membungkus SELURUH proses dalam DB Transaction
            DB::transaction(function () use ($request, $productId) {
