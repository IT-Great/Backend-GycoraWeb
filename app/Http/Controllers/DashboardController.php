...
//                     'id' => $prod ? $prod->id : random_int(1000, 9999),
//                     'name' => $item['name'],
//                     'image' => $prod ? $formatImageUrl($prod->image_url) : '',
//                     'reasons' => "Historical Best: Sold " . ($item['total_sold'] ?? 0) . " units (Fallback Mode).",
//                     'label' => 'Historical Best',
//                     'color' => 'text-blue-600',
//                     'score' => max(60, $dynamicScore)
...
//                 $appUrl = config('app.url') ? config('app.url') : 'https://back.gycoraessence.com';
//                 $baseUrlFixed = str_replace('/api', '', $appUrl);
//                 $imgUrl = $item->image_url && !str_starts_with($item->image_url, 'http') ? $baseUrlFixed . '/storage/' . $item->image_url : $item->image_url;
...
//     // =========================================================================
//     // 👇 [BARU] FUNGSI MAINTENANCE MODE (IP WHITELISTING) 👇
//     // =========================================================================
//     public function toggleMaintenance(Request $request)
//     {
//         // Pengecekan Otorisasi Admin
//         $admin = $request->user();
//         if (!$admin || !in_array($admin->usertype, ['superadmin', 'admin'])) {
...
//             Cache::forget('maintenance_whitelist_ip');
//             return response()->json([
//                 'status' => false,
//                 'message' => 'Maintenance mode dimatikan. Semua pengguna publik dapat kembali mengakses web.'
//             ]);
//         } else {
//             // Aktifkan Maintenance & Catat IP Admin
//             Cache::forever('maintenance_mode', true);
//             Cache::forever('maintenance_whitelist_ip', $request->ip());
//             return response()->json([
//                 'status' => true,
//                 'message' => 'Maintenance mode AKTIF. IP Perangkat Anda ('.$request->ip().') telah dimasukkan ke daftar pengecualian sehingga Anda tetap bisa melakukan tes.'
//             ]);
//         }
//     }
...
//             'whitelist_ip' => Cache::get('maintenance_whitelist_ip')
//         ]);
//     }
// }
...
class DashboardController extends Controller
{
    ...
    // =========================================================================
    // 👇 [BARU] FUNGSI EKSPOR LAPORAN KEUANGAN (EXCEL/CSV STREAMING) 👇
    // =========================================================================
    public function exportFinancialReport(Request $request)
    {
        // Otorisasi: Hanya Admin & Accounting yang boleh mengekspor data sensitif
        $admin = $request->user();
        if (!$admin || !in_array($admin->usertype, ['superadmin', 'admin', 'accounting'])) {
            return response()->json(['message' => 'Akses ditolak.'], 403);
        }

        // Ambil semua transaksi yang sukses/sedang proses (Bukan yang gagal/belum bayar)
        $transactions = Transaction::with(['user', 'payment'])
            ->whereIn('status', ['completed', 'processing', 'shipped', 'refunded'])
            ->orderBy('created_at', 'DESC')
            ->get();

        $fileName = 'Laporan_Finansial_Gycora_' . date('Y-m-d') . '.csv';

        // Header wajib agar browser mengenali file ini sebagai attachment unduhan (Download)
        $headers = [
        ...
        $columns = [
            'Order ID', 'Tanggal Transaksi', 'Nama Pelanggan', 'Email',
            'Total Belanja Produk', 'Ongkos Kirim', 'Diskon Promo',
            'Diskon Poin', 'Total Uang Diterima (Nett)', 'Metode Pembayaran', 'Status'
        ];

        // Menggunakan StreamedResponse agar memori server tidak penuh walau men-download 1 juta baris
        $callback = function() use($transactions, $columns) {
            $file = fopen('php://output', 'w');
            fputcsv($file, $columns); // Tulis Header Tabel

            foreach ($transactions as $t) {
                $totalBelanja = $t->total_amount;
                $ongkir = $t->shipping_cost;
                $promo = $t->promo_discount;
                // Asumsi 1 poin = Rp1.000 (Sesuaikan jika konfigurasi bisnis Anda berbeda)
                $poinRupiah = $t->points_used * 1000;

                // Menghitung Uang Bersih (Nett) yang harusnya dicocokkan dengan dashboard Xendit
                $totalNett = ($totalBelanja + $ongkir) - $promo - $poinRupiah;

                $row = [
                    $t->order_id,
                    $t->created_at->format('Y-m-d H:i:s'),
                    $t->user ? $t->user->first_name . ' ' . $t->user->last_name : 'Guest/Tamu',
                    $t->user ? $t->user->email : '-',
                    $totalBelanja,
                    $ongkir,
                    $promo,
                    $poinRupiah,
                    $totalNett,
                    $t->payment_method ?: 'Belum Terekam',
                    strtoupper($t->status)
                ];

                fputcsv($file, $row); // Tulis Baris per Baris (Sangat Ringan)
            }
