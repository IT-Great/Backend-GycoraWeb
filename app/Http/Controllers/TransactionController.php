...
// use Illuminate\Support\Facades\Cache;
// use Illuminate\Support\Facades\Storage;
// use App\Services\FraudDetectionService; // 👇 [BARU] Import Fraud Engine
...
//     //         // =========================================================================
//     //         // PANGGIL DYNAMIC PROMO ENGINE
//     //         // =========================================================================
//     //         $promoEngine = new PromoEngineService;
//     //         $dynamicPromoResult = $promoEngine->calculate($totalAmount, $hasBundleProduct);

//     //         $merdekaDiscount = $dynamicPromoResult['discount_amount'];
//     //         $freebies = $dynamicPromoResult['freebies'];
...
//     //         // =========================================================================
//     //         // 👇 [BARU] FRAUD DETECTION ENGINE ANALYZE 👇
//     //         // =========================================================================
...
//     //         $cityDest = $address ? $address->city : 'Unknown';

//     //         $fraudEngine = app(FraudDetectionService::class);
//     //         $fraudAnalysis = $fraudEngine->analyze(
//     //             $lockedUser,
//     //             $request->ip(),
//     //             $receiverName,
//     //             $totalAmount,
...
//     //         // Jika terdeteksi risiko tinggi, paksa transaksi masuk status tertahan
//     //         $finalStatus = $fraudAnalysis['is_risky'] ? 'on_hold' : 'pending';
//     //         // 👆 ================================================================== 👆
...
//     //             'order_id' => $orderId,
//     //             'total_amount' => $totalAmount,
//     //             'status' => $finalStatus, // MENGGUNAKAN STATUS FRAUD ENGINE
//     //             'point' => $earnedPoints,
//     //             'points_used' => $pointsUsed,
//     //             'promo_code' => $appliedPromoCode,
//     //             'promo_discount' => $promoDiscountAmount,
...
//     //     // PANGGIL PAYMENT GATEWAY DI LUAR DB TRANSACTION
//     //     try {
//     //         $transactionController = app(PaymentController::class);
//     //         $request->merge([
//     //             'transaction_id' => $transactionData['transaction']->id,
...
//         // 1. GENERATE TIKET ANTREAN & POSISI (GIMMICK)
//         $ticketId = 'TCK-' . $request->user()->id . '-' . Str::random(8);
//         $position = rand(150, 350); // Simulasi antrean (Misal: urutan ke-250)

//         // 2. SIMPAN TIKET KE CACHE / RAM (Berlaku 15 menit)
...
//         // 3. LEMPAR KE BACKGROUND QUEUE AGAR SERVER TIDAK DOWN
//         \App\Jobs\ProcessFlashSaleCheckout::dispatch(
//             $request->user()->id,
//             $request->all(),
//             $request->ip(), // IP diperlukan untuk Anti-Fraud
//             $ticketId
//         );

//         // 4. KEMBALIKAN NOMOR TIKET KE FRONTEND (Frontend akan Polling)
//         return response()->json([
//             'ticket_id' => $ticketId,
//             'position' => $position,
//             'status' => 'waiting'
...
//     // =========================================================================
//     // 👇 FUNGSI BARU UNTUK MENGECEK STATUS TIKET OLEH FRONTEND 👇
//     // =========================================================================
...
//         // Animasi agar urutan posisi pelan-pelan menurun
...
//     // // =========================================================================
//     // // 👇 FUNGSI BARU: MENGEKSEKUSI CHECKOUT DI LATAR BELAKANG (DIPANGGIL OLEH JOB) 👇
//     // // =========================================================================
//     // public function executeCheckoutLogic($userId, $requestData, $ipAddress, $ticketId)
//     // {
//     //     try {
//     //         // Login paksa secara background
//     //         \Illuminate\Support\Facades\Auth::loginUsingId($userId);

//     //         // Rekonstruksi Request
//     //         $request = new \Illuminate\Http\Request($requestData);
//     //         $request->server->set('REMOTE_ADDR', $ipAddress); // Inject ulang IP
...
//     //         // --- EKSEKUSI DB TRANSACTION ASLI (Semua 300 baris kode Abang) ---
//     //         $transactionData = DB::transaction(function () use ($user, $cartItems, $request) {
//     //             $lockedUser = User::lockForUpdate()->find($user->id);
...
//     //             foreach ($cartItems as $item) {
//     //                 $product = Product::with('category')->lockForUpdate()->find($item->product_id);
//     //                 if (! $product || $product->stock < $item->quantity) throw new \Exception('Stok produk '. $product->name .' telah habis di-book pelanggan lain.');
...
//     //                     if ($discountForPair > 0) {
//     //                         $halfPrice = $driver['bundle_price'] / 2;
//     //                         $itemTotals[$driver['cart_id']] += $halfPrice;
//     //                         $itemTotals[$partner['cart_id']] += $halfPrice;
//     //                     } else {
//     //                         $itemTotals[$driver['cart_id']] += $driver['normal_price'];
...
//     //                     'product_id' => $item->product_id,
//     //                     'quantity' => $item->quantity,
//     //                     'price' => $item->quantity > 0 ? ($calculatedGross / $item->quantity) : 0,
//     //                     'color' => $item->color,
//     //                 ]);
...
//     //         // 🎉 TIKET SUKSES: SIMPAN URL CHECKOUT KE RAM 🎉
//     //         Cache::put("checkout_ticket:{$ticketId}", json_encode([
//     //             'status' => 'success',
...
//     //         // ❌ TIKET GAGAL: SIMPAN ERROR KE RAM ❌
//     //         Cache::put("checkout_ticket:{$ticketId}", json_encode([
//     //             'status' => 'error',
...
//     //         // 👇 [PERBAIKAN MUTLAK] Beritahu objek Request buatan ini siapa User-nya 👇
...
//     //         // Kita pass $request (yang sudah dibind usernya) ke method PaymentController
//     //         $request->merge([
//     //             'transaction_id' => $transactionData['transaction']->id,
//     //             'currency' => 'IDR',
//     //         ]);
...
//     //         // Login paksa secara background agar facade Auth::user() tetap bekerja
//     //         \Illuminate\Support\Facades\Auth::loginUsingId($userId);

//     //         // 🌟 [PERBAIKAN MUTLAK] Rekonstruksi Request secara SEMPURNA sebagai POST Request 🌟
//     //         $request = \Illuminate\Http\Request::create('/api/checkout', 'POST', $requestData);
//     //         $request->server->set('REMOTE_ADDR', $ipAddress); // Inject ulang IP Address

//     //         // Beritahu objek Request buatan ini siapa User-nya
//     //         $request->setUserResolver(function () use ($user) {
//     //             return $user;
//     //         });

//     //         // Wajib: Daftarkan ke Global Container agar PaymentController & validasi bisa membacanya!
...
//     //         // Payment Controller sekarang bisa memanggil $request->user() dengan aman!
//     //         $invoiceRes = $transactionController->createInvoice($request);
//     //         $invoiceData = $invoiceRes->getData(true);
...
//             // 🌟 [PERBAIKAN MUTLAK] Buat instansi Request yang Murni 🌟
//             $request = new \Illuminate\Http\Request();
//             $request->replace($requestData); // Masukkan semua data JSON ke dalam payload Request
//             $request->server->set('REMOTE_ADDR', $ipAddress); // Inject ulang IP

//             // Set manual User Resolver agar fungsi-fungsi internal tetap menganggap ada user login
//             $request->setUserResolver(function () use ($user) {
//                 return $user;
//             });

//             // Wajib: Daftarkan ke Global Container!
//             app()->instance('request', $request);
...
//             // 👇 [PERBAIKAN MUTLAK] Inject user_id secara eksplisit ke dalam request payload
//             $request->merge([
//                 'transaction_id' => $transactionData['transaction']->id,
//                 'currency' => 'IDR',
//                 'user_id' => $user->id,
...
//     // =====================================================================
//     // 👇 [BARU] FUNGSI APPROVE MANUAL TRANSAKSI FRAUD (ON HOLD) 👇
//     // =====================================================================
...
//         // Setujui pesanan, ubah status menjadi pending agar pembayaran diproses
...
//     // =====================================================================
//     // FUNGSI UNTUK ADMIN MENGHAPUS TRANSAKSI PERMANEN
//     // =====================================================================
//     public function forceDeleteTransaction(Request $request, $id)
//     {
//         $transaction = Transaction::with(['details', 'payment'])->find($id);
...
//             // 🌟 REKONSTRUKSI REQUEST YANG SEMPURNA 🌟
//             $request = \Illuminate\Http\Request::create('/api/checkout', 'POST', $requestData);
...
//                 // 👇 [PERBAIKAN MYSQL ENUM] Gunakan status yang dikenal database ('pending')
//                 $finalStatus = 'pending';
...
//     // =================================================================================
//     // 🌟 [FITUR SENIOR] HAVERSINE FORMULA (Penghitung Jarak Koordinat Bumi) 🌟
//     // =================================================================================
...
//         return $angle * $earthRadius;  // Hasil dalam KM
//     }
...
//                 // ==================================================================
//                 // 🌟 [FITUR SENIOR] SMART WAREHOUSE ROUTING ALGORITHM 🌟
//                 // ==================================================================
//                 $address = \App\Models\Address::find($request->address_id);
//                 $buyerLat = $address && $address->latitude ? (float) $address->latitude : -6.2088;  // Default Jakarta
//                 $buyerLon = $address && $address->longitude ? (float) $address->longitude : 106.8456;

//                 // Definisi Multi-Warehouse Gycora (Persiapan Ekspansi)
//                 $warehouses = [
...
//                 // Hitung Jarak (Haversine) dari pembeli ke semua gudang
//                 foreach ($warehouses as &$wh) {
//                     $wh['distance'] = $this->calculateHaversineDistance($buyerLat, $buyerLon, $wh['lat'], $wh['lon']);
//                 }

//                 // Urutkan gudang berdasarkan jarak terdekat (Greedy Sort)
//                 usort($warehouses, function ($a, $b) {
//                     return $a['distance'] <=> $b['distance'];
//                 });

//                 // Variabel untuk melacak log routing pesanan
//                 $fulfillmentLog = [];
//                 // ==================================================================
...
//                     // 🌟 GREEDY ROUTING LOGIC (Mengeksekusi Split Order) 🌟
...
//                         // SIMULASI CEK STOK DI DATABASE GUDANG CABANG (Mockup Arsitektur)
//                         // Saat ini kita menggunakan master $product->stock untuk backward compatibility,
//                         // tapi algoritma ini menunjukkan cara sistem "mengambil" dari cabang terdekat.
//                         $dummyAvailableInWh = ceil($product->stock * 0.7);  // Simulasi: Cabang ini punya 70% stok
...
//                     // Gabungkan rute item ini ke Log Master
...
//                         'quantity' => $item->quantity,
//                         'price' => $item->quantity > 0 ? ($calculatedGross / $item->quantity) : 0,
//                         'color' => json_encode(['raw' => $item->color, 'routing' => $itemRoutes]),  // Simpan log rute logistik
//                     ]);
...
//                 // Log Perencanaan Routing Opsional
    ...
    public function executeCheckoutLogic($userId, $requestData, $ipAddress, $ticketId)
    {
        try {
            ...
            $transactionData = DB::transaction(function () use ($user, $cartItems, $request) {
                ...
                // Meneruskan $lockedUser sebagai referensi yang benar ke fungsi eksternal
    ...
    public function cancelOrder(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($transaction) {
            ...
            if ($lockedTransaction->status !== 'refund_manual_required' && $lockedTransaction->status !== 'cancelled') {
                ...
                //     $this->restoreProductStock($detail->product_id, $detail->quantity);
                // }
                // Mencegah Deadlock: Urutkan detail transaksi berdasarkan product_id sebelum di-loop
