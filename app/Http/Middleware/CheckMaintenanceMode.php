...
class CheckMaintenanceMode
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if (Cache::get('maintenance_mode', false)) {
            ...
            // 👇 PENGAMANAN ADMIN LOCKOUT 👇
            // 1. Izinkan jika yang mengakses adalah IP yang menyalakan tombol
            // 2. Izinkan JIKA rutenya adalah rute vital admin (login, status maintenance, toggle maintenance)
            // Sehingga admin BISA mematikan sistem dari IP manapun jika mereka sudah login.
            $isVitalAdminRoute = $request->is('api/admin/login*') ||
                                 $request->is('api/admin/maintenance/*');

            if ($request->ip() !== $whitelistedIp && !$isVitalAdminRoute) {
                return response()->json([
                    'message' => 'Sistem sedang dalam perbaikan berkala. Harap kembali beberapa saat lagi.',
                    'is_maintenance' => true
                ], 503);
